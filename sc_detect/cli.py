...
def timecode_type_check(metavar = None):
    ...
    def _type_checker(value):
        ...
        msg = ('invalid timecode: %s (timecode must conform to one of the'
               ' formats the sc_detect --help message)' % value)
...
class AboutAction(argparse.Action):
    """Custom argparse action for displaying the Pysc_detect ABOUT_STRING.
...
def get_cli_parser(scene_detectors_list, timecode_formats_list):
    """Creates the Pysc_detect argparse command-line interface.
    ...
    parser.add_argument(
        '-v', '--version',
        action = AboutAction, version = sc_detect.ABOUT_STRING)
    ...
    parser.add_argument(
        ...
        default = sc_detect.detectors.DETECTOR_DEFAULT,
    ...
    #    default = sc_detect.timecodes.FORMAT_DEFAULT,
