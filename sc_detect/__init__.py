...
#
# This file contains all code for the main `sc_detect` module.
#
...
import sc_detect.platform
import sc_detect.detectors
import sc_detect.timecodes
import sc_detect.manager
import sc_detect.cli
import sc_detect.util_img
...
Pysc_detect %s
----------------------------------------------------
Site/Updates: https://github.com/Breakthrough/Pysc_detect/
Documentation: http://pysc_detect.readthedocs.org/

Copyright (C) 2012-2017 Brandon Castellano. All rights reserved.

Pysc_detect is released under the BSD 2-Clause license. See the
included LICENSE file or visit the Pysc_detect website for details.
...
def detect_scenes_file(path, scene_manager):
    ...
    if not cap.isOpened():
        if not scene_manager.quiet_mode:
            print('[Pysc_detect] FATAL ERROR - could not open video %s.' % path)
        return (video_fps, frames_read)
    elif not scene_manager.quiet_mode:
        print('[Pysc_detect] Parsing video %s...' % file_name)
    ...
    if scene_manager.args.max_cal_width:
        scaleFactor = sc_detect.util_img.ratio_scale_factor(
            video_width,
            video_height,
            scene_manager.args.max_cal_width,
            scene_manager.args.max_cal_height)
        downscale_factor = round(1 / scaleFactor)
        scene_manager.downscale_factor = downscale_factor
        print("max cal width set to: {}, scaleFactor: {}, down: {}".format(
            scene_manager.args.max_cal_width,
            scaleFactor,
            downscale_factor))

    if not scene_manager.quiet_mode:
        print('[Pysc_detect] Video Resolution / Framerate: %d x %d / %2.3f FPS' % (
            video_width, video_height, video_fps))
        if scene_manager.downscale_factor >= 2:
            print('[Pysc_detect] Subsampling Enabled (%dx, Resolution = %d x %d)' % (
...
def detect_scenes(cap, scene_manager, image_path_prefix = '', start_frame = 0,
    ...
    while True:
        ...
        im_scaled = im_cap
        if scene_manager.downscale_factor > 1:
        ...
        if scene_manager.stats_writer:
            ...
            if len(stats_file_keys) > 0:
                scene_manager.stats_writer.writerow(
                    [str(frames_read)] +
                    [sc_detect.timecodes.frame_to_timecode(frames_read, video_fps)] +
...
def save_preview_images(scene_manager, image_path_prefix, im_curr, im_last, num_scenes):
    ...
    """
    video_height, video_width, _ = im_curr.shape
    if scene_manager.args.thumb_width:
        scale_factor = sc_detect.util_img.ratio_scale_factor(
            video_width,
            video_height,
            scene_manager.args.thumb_width,
            scene_manager.args.thumb_width)

        newWidth = int(video_width * scale_factor)
        newHight = int(video_height * scale_factor)
        im_resized = cv2.resize(im_curr, (newWidth, newHight))
    else:
        im_resized = im_curr
    ...
    output_path = os.path.join(scene_manager.args.thumb_path, output_name)
    print(output_path, im_resized.shape)
    cv2.imwrite(output_path, im_resized)


def main():
    """Entry point for running Pysc_detect as a program.
    ...
    timecode_formats = sc_detect.timecodes.get_available()
    # Parse CLI arguments.
    args = sc_detect.cli.get_cli_parser(
    ...
    if not args.quiet_mode:
        print('[Pysc_detect] Detecting scenes (%s mode)...' % smgr.detection_method)
    ...
    scene_list_tc = [sc_detect.timecodes.get_string(x) for x in scene_list_msec]
    ...
    if frames_read >= 0:
        ...
        if not args.quiet_mode:
            print('[Pysc_detect] Processing complete, found %d scenes in video.' % (
                len(smgr.scene_list)))
            print('[Pysc_detect] Processed %d / %d frames read in %3.1f secs (avg %3.1f FPS).' % (
                frames_processed, frames_read, elapsed_time, perf_fps))
            if len(smgr.scene_list) > 0:
                if args.list_scenes:
                    print('[Pysc_detect] List of detected scenes:')
                    print ('-------------------------------------------')
                    ...
                    print ('-------------------------------------------')
                print('[Pysc_detect] Comma-separated timecode output:')
...
def split_input_video(input_path, output_path, timecode_list_str):
    ...
    #args.output.close()
    print('[Pysc_detect] Splitting video into clips...')
    ret_val = None
    try:
        ...
        print('[Pysc_detect] Error: mkvmerge could not be found on the system.'
              ' Please install mkvmerge to enable video output support.')
    if ret_val is not None:
        if ret_val != 0:
            print('[Pysc_detect] Error splitting video '
                  '(mkvmerge returned %d).' % ret_val)
        else:
            print('[Pysc_detect] Finished writing scenes to output.')
